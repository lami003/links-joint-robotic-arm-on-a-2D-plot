...
L4 = 1.0 #length of link 4

def fk(theta1, theta2, theta3,theta4):
    ...
    x4 = x3 + L4*np.cos(theta1 + theta2 + theta3 +theta4) #end-effector x4 coordinate
    y4 = y3 + L4*np.sin(theta1 + theta2 + theta3 +theta4) #end-effector y4 coordinate
...
ax.set_xlim(- (L1+L2+L3+L4+0.2), L1+L2+L3+L4+0.2)
ax.set_ylim(- (L1+L2+L3+L4+0.2)+ shift_y, L1+L2+L3+L4+0.2+ shift_y)
...
theta4_0 = np.deg2rad(30.0) #convert degree to radian

# draw initial arm
base,joint1, joint2, joint3, ee = fk(theta1_0, theta2_0 , theta3_0,theta4_0)
(link_line,) = ax.plot([base[0], joint1[0], joint2[0], joint3[0], ee[0]], #plotting the arm
                       [base[1], joint1[1], joint2[1], joint3[1], ee[1]],
...
s_theta4 = Slider(slider_ax4, 'θ4 (deg)', -180.0, 180.0, valinit=np.rad2deg(theta4_0))
...
def update(_):
    ...
    th4 = np.deg2rad(s_theta4.val) #convert degree to radian
    base, joint1,joint2,joint3,e = fk(th1, th2 , th3 , th4)   #calling the forward kinematics function 
    link_line.set_data([base[0], joint1[0], joint2[0],joint3[0], e[0]],
                       [base[1], joint1[1], joint2[1],joint3[1], e[1]]) 
    ee_text.set_text(f"EE: x={e[0]:.3f}, y={e[1]:.3f}\nθ1={np.rad2deg(th1):.1f}°, θ2={np.rad2deg(th2):.1f}°, θ3={np.rad2deg(th3):.1f}°, θ4={np.rad2deg(th4):.1f}°" )
    plt.draw()
